...
export function TestimonialSection() {
  ...
  return (
    <section id="results" ref={ref} className="border-t border-border/50 bg-navy-dark py-24 md:py-36">
      <div className="mx-auto max-w-7xl px-6">
        <div className={`mb-16 transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <h2 className="font-display text-4xl leading-tight md:text-6xl">
            Proof, <em className="text-primary">not promises.</em>
          </h2>
        </div>
        ...
        <div className="grid gap-6 md:grid-cols-3">
          {testimonials.map((t, i) => (
            <div
              key={i}
              className={`flex flex-col rounded-xl border-l-4 border-primary bg-card p-8 transition-all duration-700 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"}`}
