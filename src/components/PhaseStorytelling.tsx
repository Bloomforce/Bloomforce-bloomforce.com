...
function PhaseCard({ phase, index }: { phase: typeof phases[0]; index: number }) {
  ...
  return (
    <div
      ref={ref}
      className={`relative border-l-2 border-teal/20 pl-8 pb-16 last:pb-0 transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-12"}`}
      style={{ transitionDelay: `${index * 200}ms` }}
    >
      <div className="absolute -left-[9px] top-0 h-4 w-4 rounded-full border-2 border-teal bg-navy-dark" />

      <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:gap-6">
        <span className="font-display text-6xl text-teal/20 md:text-8xl">{phase.num}</span>
        <div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {phase.tags.map((tag) => (
          <span key={tag} className="rounded-full border border-foreground/10 bg-foreground/5 px-3 py-1 font-mono text-[11px] text-muted-foreground">
            {tag}
          </span>
        ))}
      </div>
      ...
      <div className="mx-auto max-w-7xl px-6">
        <div ref={ref} className={`mb-20 max-w-3xl transition-all duration-1000 ${isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-10"}`}>
          ...
          <h2 className="font-display text-4xl leading-tight text-foreground md:text-6xl">
            ...
            <br />
            <em className="text-teal">You build your team.</em>
          </h2>
        </div>
